...
const QString CDisplayTemplateMgr::fillTemplate( const QString& name, const QString& content, Settings& settings ) {
    ...
    foreach(const CLanguageMgr::Language* lang, langMap) {
        ...
        if (!lang->abbrev().isEmpty() && btconfiguration::BtConfig::getInstance().getFontForLanguage(lang).first) {
            const QFont f = btconfiguration::BtConfig::getInstance().getFontForLanguage(lang).second;
    ...
    if (lang && !lang->abbrev().isEmpty()/*&& lang->isValid()*/) {
        const QFont standardFont = btconfiguration::BtConfig::getInstance().getDefaultFont(); //we just need a dummy lang param
        langCSS.prepend(
