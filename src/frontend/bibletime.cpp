...
std::unique_ptr<QTextToSpeech> BibleTime::createTextToSpeechInstance() {
    ...
    QString const configuredEngine = btConfig().value<QString>(QStringLiteral("GUI/ttsEngine"));
    ...
    QLocale const configuredLocale = btConfig().value<QLocale>(QStringLiteral("GUI/ttsLocale"));
    if (tts->availableLocales().contains(configuredLocale))
        tts->setLocale(configuredLocale);

    QString const configuredVoice = btConfig().value<QString>(QStringLiteral("GUI/ttsVoice"));
    QList<QVoice> const voices = tts->availableVoices();
