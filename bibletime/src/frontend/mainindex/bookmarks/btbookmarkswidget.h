...
#ifndef BTBOOKMARKSWIDGET_H
#define BTBOOKMARKSWIDGET_H
...
*/
class BtBookmarksWidget : public QTreeWidget {
	Q_OBJECT

public:
	BtBookmarksWidget(QWidget *parent);
	virtual ~BtBookmarksWidget();
...
//	virtual void addGroup( const CIndexItemBase::Type type, const QString language);
	...
	*/
	QAction* const action( const NewBtBookmarkItem::MenuAction type ) const;
	/**
...
protected slots: // Protected slots
	...
	*/
	...
	/**
	* Opens a dialog to rename the current folder.
	*/
	void renameFolder();
...
private:
	...
	*/
	const bool isMultiAction( const NewBtBookmarkItem::MenuAction type ) const;

	/**
	* A helper function for d'n'd which creates a new bookmark item when drop happens.
	*/
	void createBookmarkFromDrop(QDropEvent* event, NewBtBookmarkItem* droppedIntoItem);
